...
import { AuthError, registerWithInvite } from '../auth.js'
...
async function submit(): Promise<void> {
  ...
  busy.value = true
  try {
    const user = await registerWithInvite(code.value.trim(), displayName.value.trim(), password.value)
...
<template>
  <div class="card">
    ...
    <h1>Join Hyperion</h1>
    <p class="sub">Enter the invite code an Admin gave you.</p>

    <form class="form" @submit.prevent="submit">
      <label>Invite code <input v-model="code" type="text" required autofocus /></label>
      ...
      <button type="submit" class="primary" :disabled="busy">{{ busy ? 'Joining…' : 'Create account' }}</button>
    </form>

    <p class="hint"><RouterLink to="/login">Already have an account?</RouterLink></p>
  </div>
</template>
