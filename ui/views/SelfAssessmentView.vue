...
// ── the AI key: visible but inactive without one, never hidden (plan § AI is additive) ──
const keyInput = ref('')
...
async function saveKey(): Promise<void> {
  if (!record.user || !keyInput.value.trim()) return
  keySaving.value = true
  try {
    await saveUser({ ...record.user, aiApiKey: keyInput.value.trim() })
...
async function removeKey(): Promise<void> {
  ...
  await saveUser({ ...record.user, aiApiKey: null })
  draft.value = ''
}
...
async function generate(): Promise<void> {
  error.value = ''
  if (!record.user?.aiApiKey || entries.value.length === 0) return
  busy.value = true
  try {
    const prompt = buildSelfAssessmentPrompt(entries.value)
    draft.value = await generateSelfAssessment(record.user.aiApiKey, prompt)
  } catch (cause) {
...
<template>
  <div class="board">
    ...
    <div v-if="!record.user?.aiApiKey" class="panel gated">
      <p><b>Needs your own API key.</b></p>
      <p class="note">
        Hyperion sends the entries below directly from your browser to Anthropic, using a
        key only you hold — never through this deployment's server, never anywhere else.
        Nothing here works without one, and nothing sends anything until you add it.
      </p>
      <div class="key-form">
        <input v-model="keyInput" type="password" placeholder="sk-ant-…" autocomplete="off" />
        <button class="primary" :disabled="!keyInput.trim() || keySaving" @click="saveKey">Save key</button>
      </div>
    </div>
    ...
    <template v-else>
      <div class="panel">
        <div class="key-row">
          <span class="key-status">API key set <span class="dot">••••••••</span></span>
          <button v-if="!editingKey" class="linkbtn" @click="startEditingKey">change</button>
          <button class="linkbtn" @click="removeKey">remove</button>
        </div>
        <div v-if="editingKey" class="key-form">
          <input v-model="keyInput" type="password" placeholder="sk-ant-…" autocomplete="off" />
          <button class="primary" :disabled="!keyInput.trim() || keySaving" @click="saveKey">Save</button>
          <button class="ghost" @click="editingKey = false">Cancel</button>
        </div>
